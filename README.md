...
### (1).变量常量声明
1. 传统的var声明的for循环中，变量i是var命令声明的，在全局范围内都有效，所以全局只有一个变量。每次循环，变量i的值都会发生改变，而循环内部的i指向的就是全局的i。
而用let声明的变量仅在块级作用域内有效,每次循环的i其实都是一个新的变量。
2. es6 不存在变量提升
3. 暂时性死区：只要块级作用域存在let和const命令，这个区块对这些命令声明的变量，从一开始就形成了封闭的作用域，凡是在声明之前就使用这些变量，就会报错。即：在代码块内，使用let 声明变量之前，该变量都是不可用的。语法上叫做暂时性死区
...
### (7).函数的扩展
    ...
    1. 函数参数的默认值 
        1. 基本用法-> function/函数参数默认值.html
        2. 与解构赋值默认值结合使用 -> function/与解构赋值默认值结合使用.html
        3. 参数默认值的位置 
        ...
        5. 作用域
            1. 一旦设置了参数的默认值，函数进行声明初始化时，参数会形成一个单独的作用域（context）。等到初始化结束，这个作用域就会消失。 ->function/参数默认值的作用域.html
            2. 应用 -> function/参数默认值的应用.html
            利用参数默认值，可以指定某一个参数不得省略，如果省略就抛出一个错误。
            可以将参数默认值设为undefined，表明这个参数是可以省略的。
    2. rest 参数 -> function/rest参数.html
	...
	2. arguments问题=>不能使用了
	3. 函数给默认值->之前也能用https://60.205.111.27:8080/svn/Project/Code/Web_Code
...
####9.import()
    ...
    所以import语句放在if代码块之中毫无意义，因此会报句法错误，而不是执行时错误。也就是说，import和export命令只能在模块的顶层，不能在代码块之中（比如，在if代码块之中，或在函数之中）。
    如果import命令要取代 Node 的require方法，这就形成了一个障碍。因为require是运行时加载模块，import命令无法取代require的动态加载功能。

    import()类似于 Node 的require方法，区别主要是前者import是异步加载，后者require是同步加载。

####10.import()  使用场合

    1. 按需加载
        import()可以在需要的时候，再加载某个模块。 参看import-sections-1.js
	2. 条件加载
		import()可以放在if代码块，根据不同的情况，加载不同的模块。参看import-sections-2.js
	3. 动态的模块路径
		import()允许模块路径动态生成。参看import-sections-3.js
		根据函数f的返回结果，加载不同的模块。

####11.import() 注意点 import-point1.js

	1. import()加载模块成功以后，这个模块会作为一个对象，当作then方法的参数。因此，可以使用对象解构赋值的语法，获取输出接口。 eg1
	
	2. 如果模块有default输出接口，可以用参数直接获得。eg2
	
	3. 上面的代码也可以使用具名输入的形式。 eg3

	4. 如果想同时加载多个模块，可以采用下面的写法。eg4
...
### (11).Module的加载实现
	...
	1. 浏览器加载
        ...
        3. 对于外部的模块脚本,有几点需要注意
            ...
            3. 模块之中，可以使用import命令加载其他模块（.js后缀不可省略，需要提供绝对URL或者相对URL）,也可以使用export命令输出对外接口。
            4. 模块之中，顶层的this关键字返回undefined，而不是指向window。也就是说，在模块顶层使用this关键字，是无意义的。
            5. 同一个模块如果加载多次，将只执行一次。
            6. 利用顶层的this等于undefined这个语法点，可以侦测当前代码是否在 ES6 模块之中。 -> const isNotModuleScript = this !== undefined;+
